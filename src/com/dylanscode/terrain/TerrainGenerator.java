...
public class TerrainGenerator
{
	...
	private class Cell
	{
		...
		/**
		 * noise is between 0 and 1, so we must allow colors for those colors
		 * @param noise
		 * @return
		 */
		private Color generateColor(float noise) {
			...
			}else if(noise > .25 && noise <= .42) {
				//light blue
				return new Color(3,65,165);
			}else if(noise > .42 && noise <= .45) {
				//sandy color
				return new Color(242, 240, 152);
			}else if(noise >.45 && noise <=.55) {
				//light green
				return new Color(48, 168, 42);
			}else if(noise > .55 && noise <= .6) {
				//middle green
				return new Color(4, 135, 0);
			}else if(noise > .6 && noise <= .7) {
				//dark green
				return new Color(9,89,5);
			}
			else if(noise > .7 && noise <= .78) {
				//dark gray
				return new Color(178, 178, 178);
			}else if(noise > .78 && noise <= .85) {
				//light gray
				return new Color(91, 91, 89);
			}
			else if(noise > .85 && noise <= .95) {
				//very light gray
